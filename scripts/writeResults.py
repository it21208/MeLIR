...
def writeResults(out_folder, clf, dict_initialScoreRankingResults, LambdaParam, topic_list, list_doc_score):
    ...
    def normalisation(score):
        ...
        # Check if I am dividing by zero and handle it.
        try: score[:] = [x / temp_var for x in score]
        except: score[:] = [x for x in score]
        return(score)
    ...
    # Use dict_initialScoreRankingResults and list_doc_score. Iterate through all topics in the topic_list
    for idx, topic in enumerate(topic_list):

        old_score, new_score = ([] for i in range(2))

        # Loop through the items in the list_doc_score dictionary. for key, value in (list_doc_score_copies[idx])[idx].items():
        ...
        sorted_new_doc_score = sorted(list_doc_score[idx].items(), key=operator.itemgetter(1))
