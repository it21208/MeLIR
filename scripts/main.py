...
class main:
    ...
    def steps2_and_3(self, topic, rows_seed, qrels_filepath, cols_seed, data_seed, qrels_content_filepath, list_cols_seed_and_feature_names, dict_initialScoreRankingResults):
        ...
        ordered_feature_names = [[i[0], i[2]] for i in ordered_list_cols_seed_and_feature_names]
        docs_reviewed = set()
    ...
    def augmentTrainSetWithRandomDocs_splitTraiTest_trainClf(self, list_of_pmids_for_topic, training_set, inverted_lookup_dict, topic, pathToFeatures, tmp_lst, clf, data_seed, new_rows_seed, doc_score, cols_seed, lookup_dict, vocab_idx_dict, tfidf_dict, ordered_feature_names):
        ...
        # Get a list of the train PMIDs in the training set.
        training_set_list_of_pmids = [inverted_lookup_dict[i] for i in training_set]
        X_train = [key for key, value in training_set.items()]
        y_train = [value for key, value in training_set.items()]
        # Get a list of the test PMIDs in the testing set
        ...
        for i, j, v in zip(cx.row, cx.col, cx.data):
            try:
                if lookup_dict[i] in new_train_set:  y_train_dict[i] = y[i]
                y_test_dict[i] = y[i]
        ...
        # Combine y_test and X_test in a data structure. - Here there might be an error.
...
def run_main(projDir, seedDoc_filepath, qrels_filepath, tfidf_dict, vocab_idx_dict, topic_docid_label, clf, tfidf_sp, pathToFeatures, docid_idx_dict, topic, qrels_content_filepath, idf_dict, vocab_idx_dict_2, cur_idx, total_words, idf_word_dict, dict_initialScoreRankingResults):
    # Instantiate an object from the class main. And theb the object implements the method constructSyntheticDoc of the class.
    # The object implements the method steps2_and_3 of the class. This implements the 2nd & 3rd step of the AutoTAR method which are executed once for each topic.
    # Afterwards the object implements the method cal of the class. And finally it returns to run.py an ordered dictionary with the doc scores for each topic.
